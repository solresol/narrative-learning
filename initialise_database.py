...
def _execute(cur, query: str, params: tuple = (), postgres: bool = False) -> None:
...
def generate_schema_sql(obfuscation_plan, source_df, postgres: bool = False, dataset_prefix: str = ""):
    """
  ...
  creation_time {'timestamp' if postgres else 'datetime'} default current_timestamp,
  ...
  primary key (round_id, {obfuscated_primary_key})
    ...
    autoinc = "integer generated by default as identity" if postgres else "integer primary key autoincrement"
    schema_sql += f"create table if not exists splits (\n  split_id {autoinc}\n);\n\n"
  ...
  primary key (split_id, {obfuscated_primary_key})
    ...
    schema_sql += f"  round_id {'integer generated by default as identity' if postgres else 'integer primary key autoincrement'},\n"
    schema_sql += f"  round_start {'timestamp' if postgres else 'datetime'} default current_timestamp,\n"
...
def main():
    ...
    schema_sql = generate_schema_sql(obfuscation_plan, source_df, postgres, dataset_prefix)
    if args.verbose:
    ...
    # Create a new split
    cur = conn.cursor()
    _execute(cur, "INSERT INTO splits DEFAULT VALUES RETURNING split_id", postgres=postgres)
    row = cur.fetchone()
    split_id = row[0]
    
    # Insert a new round
    rounds_table = f"{dataset_prefix}_rounds" if dataset_prefix else "rounds"
    _execute(cur, f"INSERT INTO {rounds_table} (prompt, split_id) VALUES (?, ?)", (args.prompt, split_id), postgres)
    
    # Create column list for insert query
    columns = ["decodex"] + [col for col in obfuscated_df.columns]
    placeholders = ", ".join(["?" if not postgres else "%s"] * len(columns))
    ...
    for index, row in obfuscated_df.iterrows():
        ...
        _execute(cur, insert_sql, tuple(values), postgres)
        
        # Handle holdout and validation
        holdout = random.random() < args.holdout
        validation = holdout and (random.random() < args.validation)
        _execute(cur, f"INSERT INTO {obfuscated_split_table_name} (split_id, {obfuscated_primary_key}, holdout, validation) VALUES (?, ?, ?, ?)",
                 (split_id, row[obfuscated_primary_key], holdout, validation), postgres)
